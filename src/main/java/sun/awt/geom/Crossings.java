...
public abstract class Crossings {
	...
	public final static class EvenOdd extends Crossings {
		...
		public void record(double ystart, double yend, int direction) {
			...
			if (to < from && from < limit) {
				Helper.arraycopy(yranges, from, yranges, to, limit - from);
			}
			to += (limit - from);
			if (ystart < yend) {
				if (to >= yranges.length) {
					double newranges[] = new double[to + 10];
					Helper.arraycopy(yranges, 0, newranges, 0, to);
					yranges = newranges;
				}
				yranges[to++] = ystart;
				yranges[to++] = yend;
	...
	public final static class NonZero extends Crossings {
		...
		public void remove(int cur) {
			...
			int rem = limit - cur;
			if (rem > 0) {
				Helper.arraycopy(yranges, cur + 2, yranges, cur, rem);
				Helper.arraycopy(crosscounts, cur / 2 + 1, crosscounts, cur / 2, rem / 2);
			}
		}
		...
		public void insert(int cur, double lo, double hi, int dir) {
			...
			if (limit >= yranges.length) {
				...
				Helper.arraycopy(oldranges, 0, yranges, 0, cur);
				crosscounts = new int[(limit + 10) / 2];
				Helper.arraycopy(oldcounts, 0, crosscounts, 0, cur / 2);
			}
			if (rem > 0) {
				Helper.arraycopy(oldranges, cur, yranges, cur + 2, rem);
				Helper.arraycopy(oldcounts, cur / 2, crosscounts, cur / 2 + 1, rem / 2);
			}
			yranges[cur + 0] = lo;
			yranges[cur + 1] = hi;
